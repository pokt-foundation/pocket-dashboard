...
export const GetTotalRangedRelaysAndLatency = gql`
    query getTotalRangedRelaysAndLatency($_eq: String, $_gte: timestamptz, $_lte: timestamptz) {
  relay_apps_daily_aggregate(
...
export const GetTotalSuccessfulRelays = gql`
  ...
  relay_apps_daily_aggregate(
    where: {app_pub_key: {_eq: $_eq}, bucket: {_gte: $_gte}, result: {_eq: "200"}}
    order_by: {bucket: desc}
  ) {
    aggregate {
      sum {
...
export const GetTotalSuccessfulRangedRelays = gql`
    query getTotalSuccessfulRangedRelays($_eq: String, $_gte: timestamptz, $_lte: timestamptz) {
  relay_apps_daily_aggregate(
    where: {app_pub_key: {_eq: $_eq}, bucket: {_gte: $_gte, _lte: $_lte}, result: {_eq: "200"}}
    order_by: {bucket: desc}
  ) {
    aggregate {
      sum {
...
export const GetTotalRelayDuration = gql`
  ...
  relay_app_hourly(
    where: {app_pub_key: {_eq: $_eq}, bucket: {_gte: $_gte}, elapsed_time: {_lte: "3"}}
    order_by: {bucket: desc}
  ) {
    elapsed_time
    bucket
